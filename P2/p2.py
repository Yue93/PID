...
def alineacion(image,rgbCopia,filtro,escala):
    ...
    channelR=cropImg[:,:,0]
    print "Dimension del canal R:",channelR.shape
    channelRGradient=np.array(np.gradient(channelR,2))
    print "Dimension del gradiente del canal R:",channelRGradient.shape
    for i in range(2):  
        cropGradient=np.array(np.gradient(cropImg[:,:,i+1]))
        print "Dimension del gradiente del imagen crop:",cropGradient.shape
